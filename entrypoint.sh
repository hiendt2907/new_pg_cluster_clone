...
init_primary() {
  ...
  gosu postgres psql -U "$POSTGRES_USER" <<-EOSQL
    -- Revoke any existing permissions first to ensure clean state
    ...
    REVOKE ALL ON DATABASE postgres FROM app_readonly;
    
    -- Database and schema access (read-only)
    GRANT CONNECT ON DATABASE postgres TO app_readonly;
    GRANT USAGE ON SCHEMA public TO app_readonly;
    
    -- Table permissions: SELECT ONLY (existing and future)
    GRANT SELECT ON ALL TABLES IN SCHEMA public TO app_readonly;
    ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO app_readonly;
    
    -- Sequence permissions: SELECT ONLY (for reading SERIAL values)
    ...
    -- Function permissions: EXECUTE only
...
EOSQL
  ...
  log "Application users created with full permissions"
...
attempt_rewind() {
  ...
  if gosu postgres pg_rewind --target-pgdata="$PGDATA" \
      --source-server="host=$host port=$port user=$REPMGR_USER dbname=$REPMGR_DB password=$REPMGR_PASSWORD"; then
